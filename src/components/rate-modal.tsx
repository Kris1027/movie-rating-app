...
export function RateModal({
  ...
  return (
    <div className="flex justify-center items-center fixed top-0 left-0 w-full h-screen bg-black bg-opacity-50 z-50">
      <div className="bg-secondary px-48 py-20 rounded-md text-center relative">
        ...
        <div className="absolute -top-16 left-1/2 transform -translate-x-1/2">
          <Star color="#2d7dd2" fill="#2d7dd2" size={148} />
          <p className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 text-3xl">
            {selected ? selected : "?"}
          </p>
        </div>
        <div>
          <p className="uppercase text-[#facc15] font-bold py-4">rate this</p>
          <Heading>
            {contentType === "movies"
